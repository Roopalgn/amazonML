...
from src.submission.make_scored_outputs import PAIR_FEATURE_COLUMNS
...
def feature_row(row: dict[str, str]) -> np.ndarray:
    return np.fromiter((float(row[name]) for name in PAIR_FEATURE_COLUMNS), dtype=np.float32)
...
def collect_reservoir(
    ...
    *,
    ...
    arrays = {0: np.empty((cap_per_class, len(PAIR_FEATURE_COLUMNS)), dtype=np.float32),
              1: np.empty((cap_per_class, len(PAIR_FEATURE_COLUMNS)), dtype=np.float32)}
    seen = {0: 0, 1: 0}
    used = {0: 0, 1: 0}
    with scores_path.open(encoding="utf-8", newline="") as handle:
        reader = csv.DictReader(handle, delimiter="\t")
        expected = {"source1_entity_id", "candidate_entity_id", *PAIR_FEATURE_COLUMNS}
        missing = expected - set(reader.fieldnames or [])
        if missing:
            raise ValueError(f"{scores_path} missing feature column(s): {sorted(missing)}")
        for row in reader:
            source1_id = row["source1_entity_id"]
            if is_holdout(source1_id) != heldout:
                continue
            label = int(row["candidate_entity_id"] in truth[source1_id])
            seen[label] += 1
            if used[label] < cap_per_class:
                arrays[label][used[label]] = feature_row(row)
                used[label] += 1
            else:
                replace_at = rng.randrange(seen[label])
                if replace_at < cap_per_class:
                    arrays[label][replace_at] = feature_row(row)
    if not used[0] or not used[1]:
        raise ValueError(f"Need positive and negative examples; observed {seen}")
    x = np.concatenate((arrays[0][: used[0]], arrays[1][: used[1]]), axis=0)
    y = np.concatenate((np.zeros(used[0], dtype=np.float32), np.ones(used[1], dtype=np.float32)))
    return x, y, {"negative_pairs_seen": seen[0], "positive_pairs_seen": seen[1],
                  "negative_pairs_sampled": used[0], "positive_pairs_sampled": used[1]}
...
def tune_holdout(
    ...
    *,
    ...
    features_by_name = list(PAIR_FEATURE_COLUMNS)
    ...
    for source1_id in truth:
        if is_holdout(source1_id) and source1_id not in seen_source1_ids:
...
def main() -> int:
    ...
    result = {
        "model": "numpy_logistic_pair_matcher_v1",
        ...
        "heldout_split": "source1 numeric ID modulo 5 equals zero",
        "heldout_best": best,
